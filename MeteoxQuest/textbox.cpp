...
Textbox::Textbox(sf::Vector2f& pos, sf::Vector2f& size, std::string back_text)
	: MenuElement(pos, size, true)//, text_("", Game::font, 8), back_text_(back_text, Game::font, 8)
{
	...
	back_text_.setOrigin(back_text_.getCharacterSize() * back_text_.getString().getSize() * 0.5f, back_text_.getCharacterSize() * 0.5f);
	...
	if (focused_element_ == this)
	{
		...
		case sf::Event::KeyPressed:
			...
			case sf::Keyboard::BackSpace:
				current_string_ = current_string_.substring(0, current_string_.getSize() - 1);
				break;
...
{
	text_.setOrigin(text_.getCharacterSize() * text_.getString().getSize() * 0.5f, text_.getCharacterSize() * 0.5f);
}
...
void Textbox::keep_text_inside()
{
	...
	{
		while (current_char_size * text_length < click_box_.width - text_.getCharacterSize())
		{
			current_char_size++;
		};
	}
